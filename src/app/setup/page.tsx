    ...
    {tab === "goals" && <div className="section stack">
      <section className="card tinted-card">
        ...
        <div className="form-grid two-col section">
          ...
          <div className="field"><label>Stop projection when</label><select value={state.goals.stopMode} onChange={(e) => patch("goals", { stopMode: e.target.value as typeof state.goals.stopMode })}><option value="body-fat">Body-fat target is reached</option><option value="weight">Weight target is reached</option><option value="either">Either target is reached first</option><option value="both">Both targets are reached</option></select><small>This controls the endpoint; your primary target still controls which field is manually entered.</small></div>
          <div className="field"><label>After reaching it</label><select value={state.goals.onTarget} onChange={(e) => patch("goals", { onTarget: e.target.value as typeof state.goals.onTarget })}><option value="stop">Stop projection</option><option value="maintenance">Transition to maintenance</option></select></div>
        </div>
